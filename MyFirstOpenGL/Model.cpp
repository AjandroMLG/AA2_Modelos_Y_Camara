...
Model::Model(const std::vector<float>& vertexs, const std::vector<float>& uvs, const std::vector<float>& normals, Textura* textura)
{
...
Model::Model(const std::vector<float>& vertexs, Textura* textura)
{
    this->textura = textura;
    //Almaceno la cantidad de vertices que habra
    this->numVertexs = vertexs.size() / 3;
...
void Model::Render() const {
    ...
    if(textura != nullptr)
        ...
        glDrawArrays(GL_TRIANGLE_STRIP, 0, this->numVertexs);
