...
public:
	...
	static Model LoadOBJModel(const std::string& filePath, GLuint program, glm::vec3 position, glm::vec3 rotation, float rotationValue, glm::vec3 scale, glm::vec4 color) {
		...
		Model model(vertexs, textureCoordinates, vertexNormal, program, position, glm::vec3(0,1,0), MatrixTools::RandonNumberBetween(0,360), glm::vec3(MatrixTools::RandomNumberBetweenFloat(4.f,6.f)), color);
		return model;
	}
};